...
func downgradeUserGroupForSubscriptionTx(tx *gorm.DB, sub *UserSubscription, now int64) (string, error) {
	...
	chainGroups := getPaymentAutoSwitchGroupChainSet()
	if currentGroup != upgradeGroup && !isPaymentAutoSwitchGroupChainMember(currentGroup, chainGroups) {
		return "", nil
	}
	...
	targetGroup := activeUpgradeGroup
	if targetGroup == "" {
...
func ExpireDueSubscriptions(limit int) (int, error) {
	...
	for userId := range userIds {
		...
		err := DB.Transaction(func(tx *gorm.DB) error {
			...
			chainGroups := getPaymentAutoSwitchGroupChainSet()
			if currentGroup != expiredUpgradeGroup && !isPaymentAutoSwitchGroupChainMember(currentGroup, chainGroups) {
				return nil
			}
