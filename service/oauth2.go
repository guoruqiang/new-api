...
func ensureDefaultOAuthClient() error {
	...
	status := model.OAuthClientStatusDisabled
	if settings.Enabled {
		status = model.OAuthClientStatusEnabled
	}
	var clientSecret *string
	if secret := strings.TrimSpace(settings.ClientSecret); secret != "" {
		clientSecret = common.GetPointer(secret)
	}
	return model.EnsureOAuthClient(&model.OAuthClient{
		Name:          strings.TrimSpace(settings.ClientName),
...
func IsOAuthServerEnabled() bool {
...
func GetOAuthClient(clientID string) (*model.OAuthClient, error) {
	if !IsOAuthServerEnabled() {
		return nil, oauthError("server_disabled", 404, "oauth authorization server is disabled")
	}
	clientID = strings.TrimSpace(clientID)
	if clientID == "" {
		return nil, oauthError("invalid_client", 401, "client_id is required")
	}
	if clientID != strings.TrimSpace(system_setting.GetOAuthServerSettings().ClientID) {
...
func ValidateOAuthAccessToken(rawToken string, requiredScopes []string) (*ValidatedOAuthAccessToken, error) {
	...
	return &ValidatedOAuthAccessToken{User: user, Token: record, Scopes: scopes}, nil
}
...
func BuildOAuthUserInfo(user *model.User, scopes []string) map[string]any {
	result := map[string]any{"sub": strconv.Itoa(user.Id)}
	if hasScope(scopes, "profile") {
		name := strings.TrimSpace(user.DisplayName)
		if name == "" {
			name = user.Username
