...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		topUp, completed, err := model.RechargeEpay(verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP())
		if err != nil {
		...
		if completed {
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值成功 trade_no=%s user_id=%d callback_type=%s client_ip=%s topup=%q", topUp.TradeNo, topUp.UserId, verifyInfo.Type, c.ClientIP(), common.GetJsonString(topUp)))
		}
	} else {
