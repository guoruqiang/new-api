...
- 额度入账和自动切组在同一事务内完成，保留上游钱包上限校验和重复回调幂等处理；事务提交后同步余额与分组缓存。
- 事务内读取数据库时间时复用当前事务连接，避免单连接池下发生死锁。

## 4. Epay 完成时间

Epay 异步通知通过上游 `model.RechargeEpay` 原子完成入账、写入 `CompleteTime` 和自动切组，保证“只统计启用后新充值”的时间过滤有可靠数据；重复通知不重复入账。

- 实现：[`model/topup.go`](../model/topup.go)、[`controller/topup.go`](../controller/topup.go)
...
7. 数据库升级仅在副本和专用测试库验证，覆盖新库、旧库升级及重复启动；未完成 MySQL/PostgreSQL 验证时，不宣称已完成三数据库兼容性验证。
